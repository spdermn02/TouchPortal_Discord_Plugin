...
// full list of users not being updated properly when user joins?
...
const voiceChannel = async (data) => {
  ...
  if ( data.channel_id == null ) {
    logIt("DEBUG","No Channel Found - Clearing Voice Channel States")
    // await userStateHandler.clearUserStates();
    ...
    if (!DG.channels[data.guild_id] || !DG.channels[data.guild_id].voice) {
      // if user changes channels while plugin is booting, it will error/crash without this.. 
      try {
    ...
    if(DG.voice_channel_id !== '<None>' ) {
      ...
      await Promise.all(channel.voice_states.map(async (vs) => {
        if (vs.user.id !== DG.Client.user.id) {
          ...
          if (vs.user.avatar === null) {
            vs.user.base64Avatar = DG.DEFAULTBASE64_AVATAR;
    ...
    Object.keys(DG.currentVoiceUsers).forEach((key,i) => {
      ...
      if (DG.currentVoiceUsers[key].user.id !== DG.Client.user.id) {
        ...
        TPClient.stateUpdateMany([
          ...
          {id: [`user_${i}_mute`], value: DG.currentVoiceUsers[key].mute ? "On" : "Off"},
          {id: [`user_${i}_deaf`], value: DG.currentVoiceUsers[key].voice_state.deaf ? "On" : "Off"},
          {id: [`user_${i}_self_deaf`], value: DG.currentVoiceUsers[key].voice_state.self_deaf ? "On" : "Off"},
          {id: [`user_${i}_self_mute`], value: DG.currentVoiceUsers[key].voice_state.self_mute ? "On" : "Off"},
          {id: [`user_${i}_server_mute`], value: DG.currentVoiceUsers[key].voice_state.mute ? "On" : "Off"},
          {id: [`user_${i}_avatar`], value: DG.currentVoiceUsers[key].user.base64Avatar},
          {id: [`user_${i}_avatarID`], value: DG.currentVoiceUsers[key].user.avatar},
...
async function subscribeToEvents(channelId) {
    ...
    for (let event of events) {
      ...
      DG.last_voice_channel_subs.push({ subscription, description: event.description });
...
const unsubscribeFromEvents = async () => {
    if (DG.last_voice_channel_subs.length > 0) {
      ...
      for(let i = 0; i < DG.last_voice_channel_subs.length; i++) {
        logIt("INFO", `Unsubscribing from ${DG.last_voice_channel_subs[i].description}`);
        // await DG.last_voice_channel_subs[i].unsubscribe();
        // took off await and it fixed the unsubscribe issue/subscribe
        DG.last_voice_channel_subs[i].subscription.unsubscribe();
