...
class UserStateHandler {
    ...
    updateUserStates = async (data) => {
      ...
      try {
        ...
        // we should be doing a stateupdateMany here... 
        updates.push({ id: `user_${userIndex}_deaf`, value: user.voice_state.deaf ? "On" : "Off" });
        updates.push({ id: `user_${userIndex}_self_deaf`, value: user.voice_state.self_deaf ? "On" : "Off" });
        updates.push({ id: `user_${userIndex}_self_mute`, value: user.voice_state.self_mute ? "On" : "Off" });
        updates.push({ id: `user_${userIndex}_mute`, value: user.mute ? "On" : "Off" });
        updates.push({ id: `user_${userIndex}_server_mute`, value: user.voice_state.mute ? "On" : "Off" });
        updates.push({ id: `user_${userIndex}_mute`, value: user.mute ? "On" : "Off" });
        updates.push({ id: `user_${userIndex}_id`, value: user.user.id });
        updates.push({ id: `user_${userIndex}_nick`, value: user.nick });
        updates.push({ id: `user_${userIndex}_volume`, value: Math.round(user.volume) });
        ...
        if (DG.customVoiceAcivityUsers.hasOwnProperty(data.user.id)) {
          ...
          let updates = [
            { id: `${userIndex}_id`, value: DG.currentVoiceUsers[data.user.id].user.id },
            { id: `${userIndex}_nick`, value: DG.currentVoiceUsers[data.user.id].nick },
            { id: `${userIndex}_avatar`, value: DG.currentVoiceUsers[data.user.id].user.base64Avatar },
            { id: `${userIndex}_avatarID`, value: DG.currentVoiceUsers[data.user.id].user.avatar},
            { id: `${userIndex}_mute`, value: DG.currentVoiceUsers[data.user.id].mute ? "On" : "Off" },
            { id: `${userIndex}_deaf`, value: DG.currentVoiceUsers[data.user.id].voice_state.deaf ? "On" : "Off" },
            { id: `${userIndex}_self_deaf`, value: DG.currentVoiceUsers[data.user.id].voice_state.self_deaf ? "On" : "Off" },
            { id: `${userIndex}_self_mute`, value: DG.currentVoiceUsers[data.user.id].voice_state.self_mute ? "On" : "Off" },
            { id: `${userIndex}_server_mute`, value: DG.currentVoiceUsers[data.user.id].voice_state.mute ? "On" : "Off" },
            { id: `${userIndex}_volume`, value: Math.round(DG.currentVoiceUsers[data.user.id].volume) }
          ];
          TPClient.stateUpdateMany(updates);
        
        } 
    } catch (error) {
      logIt("Error in updateUserStates: " + error, "ERROR");
    }
    };
    ...
    deleteUserStates = async (data) => {
      if (DG.Client.user.id === data.user.id) {
        console.log("Client User has left the voice channel")
        await userStateHandler.clearUserStates();
      } else { 
        console.log(`${data.nick} (${data.user.id}) has left the voice channel`)
    ...
    repopulateUserStates = async () => {
      ...
      for (let userId in DG.currentVoiceUsers) {
        ...
        updates.push({ id: `user_${userIndex}_self_deaf`, value: user.voice_state.self_deaf ? "On" : "Off" });
        updates.push({ id: `user_${userIndex}_self_mute`, value: user.voice_state.self_mute ? "On" : "Off" });
        updates.push({ id: `user_${userIndex}_mute`, value: user.voice_state.mute ? "On" : "Off" });
        updates.push({ id: `user_${userIndex}_id`, value: user.user.id });
        updates.push({ id: `user_${userIndex}_nick`, value: user.nick });
        updates.push({ id: `user_${userIndex}_volume`, value: Math.round(user.volume) });
        updates.push({ id: `user_${userIndex}_avatar`, value: user.user.base64Avatar });
        updates.push({ id: `user_${userIndex}_avatarID`, value: user.user.avatar });
